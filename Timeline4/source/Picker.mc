...
class TextPickerView extends Ui.View {
    ...
    var DEVICE_WIDTH = System.getDeviceSettings().screenHeight;
    var DEVICE_HEIGHT = System.getDeviceSettings().screenWidth;
    ...
    static const FONT_PICKER_TITLE = Gfx.FONT_TINY;
    static const FONT_PICKER_SUBTITLE = Gfx.FONT_XTINY;
...
class TextPickerDelegate extends Ui.BehaviorDelegate {
    ...
    var LONG_PRESS_UPDATE_FREQUENCY = 250;
    ...
    public function onKeyPressed(event) {
        var key = event.getKey();
    ...
    System.println(type);
        ...
        if (picker.input_chars.substring(picker.charIdx,picker.charIdx+1).equals("*")) {
            ...
            if (inputStr.substring(0,3).find("MON") != null || inputStr.substring(0,3).find("TUE") != null || inputStr.substring(0,3).find("WED") != null || inputStr.substring(0,3).find("THU") != null || inputStr.substring(0,3).find("FRI") != null || inputStr.substring(0,3).find("SAT") != null || inputStr.substring(0,3).find("SUN") != null)
            {type = "timer";}
            else { type = "reminder";}
